...
- `session.append(type, data, opts?)` snapshots and freezes durable data and surface metadata, validates marker shape, cited source-event seqs, complete replacement coverage, and content-only single-result `tool/result` rewrites, commits synchronously, then notifies observers with independent failure containment. Reentrant attached-session appends reject, and runtime checks cover widened unions and loaded logs. Surface events (`user/message`, `assistant/message`, `tool/result`) take a mandatory `SurfaceIntent`; non-surface events accept an optional `AppendOpts` envelope — pass `{ ignorable: true }` to mark a purely informational record a reader may safely skip when it does not recognize the type (the out-of-tree plugin forward-compatibility contract; see the envelope docs on `SessionEvent.ignorable`). Appending an unknown non-ignorable type warns immediately at the write site, because such an event would otherwise poison the log and refuse resume on every later build.

...
function validateSessionHeader(id: SessionId, input: unknown): SessionHeader {
  ...
  if (record.cwd !== undefined) {
    ...
    // Accept either platform's absolute-path shape: a session restored on a
    // different OS still carries the cwd written by its originating host
    // (e.g. a Windows-created `C:\...` read on POSIX) (#2405).
